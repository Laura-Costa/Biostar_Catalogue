...
cursor.execute("select {father_table}.HIP as HIP_order "
         "from {father_table} "
         "where {father_table}.simbad_DR3 is not null "
...
cont = 0
print(len(HIP_list))
with open("/home/lh/Desktop/Biostar_Catalogue/Biostar_Catalogue/output_files/hipparcos/csv/estrelas_com_dr3.txt", "w") as text_file:
    for (HIP,) in HIP_list:
        cont += 1
        lista = [HIP[4:]]
