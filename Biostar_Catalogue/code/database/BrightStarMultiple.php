<?php
    ...
    while($row = mysqli_fetch_array($query)) {
        ...
        // pular os regitros que correspondem a apenas uma componente
        if(strlen($ADS_Comp) <= 1 && !str_contains($ADS_Comp, "/")) {continue};
        ...
        foreach($lines_array as $line){
            for($i=1; $i<=$number_of_objects; $i++){
                ...
                if($line[0] == $i){
                    $identifier = substr($line, strpos($line, "|")+1);
                    $identifier = substr($identifier, strpos($identifier, "|")+1);
                    $identifier = trim(substr($identifier, 0, strpos($identifier, "|")-1));

                    // aqui eu tenho o identificador da múltipla
                    mysqli_query($conn, "insert into BrightStarMultiple(identifier) values('" . $identifier . "')");
                    $last_ordinal_number++;
