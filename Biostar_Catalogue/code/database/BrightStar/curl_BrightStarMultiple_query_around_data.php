    ...
    while($row = mysqli_fetch_array($query)) {
        ...
        */
        ...
        if(!is_null($simbad_DR1)){
            mysqli_query($conn, "update BrightStarMultiple set simbad_DR1 = '" . $simbad_DR1 . "' where ordinal_number = " . $ordinal_number);
        }
        if(!is_null($simbad_DR2)){
            mysqli_query($conn, "update BrightStarMultiple set simbad_DR2 = '" . $simbad_DR2 . "' where ordinal_number = " . $ordinal_number);
        }
        if(!is_null($simbad_DR3)){
            mysqli_query($conn, "update BrightStarMultiple set simbad_DR3 = '" . $simbad_DR3 . "' where ordinal_number = " . $ordinal_number);
        }
        if(!is_null($simbad_HIP)){
            mysqli_query($conn, "update BrightStarMultiple set simbad_HIP = '" . $simbad_HIP . "' where ordinal_number = " . $ordinal_number);
        }
        if(!is_null($simbad_parallax) && !str_contains($simbad_parallax, "~")){
            $parallax_query = ("update BrightStarMultiple set simbad_parallax = " . $simbad_parallax . " where ordinal_number = " . $ordinal_number);
            mysqli_query($conn, $parallax_query);
        }
        if(!is_null($simbad_parallax_error) && !str_contains($simbad_parallax_error, "~")){
            $parallax_error_query = ("update BrightStarMultiple set simbad_parallax_error = " . $simbad_parallax_error . " where ordinal_number = " . $ordinal_number);
            mysqli_query($conn, $parallax_error_query);
        }
        if(!is_null($simbad_parallax_source) && !str_contains($simbad_parallax_source, "~")){
            $simbad_parallax_source_query = ("update BrightStarMultiple set simbad_parallax_source = '" . $simbad_parallax_source . "' where ordinal_number = " . $ordinal_number);
            mysqli_query($conn, $simbad_parallax_source_query);
        }
        if(!is_null($simbad_B) && !str_contains($simbad_B, "~")){
            $simbad_B_query = ("update BrightStarMultiple set simbad_B = " . $simbad_B . " where ordinal_number = " . $ordinal_number);
            mysqli_query($conn, $simbad_B_query);
        }
        if(!is_null($simbad_V) && !str_contains($simbad_V, "~")){
            $simbad_V_query = ("update BrightStarMultiple set simbad_V = " . $simbad_V . " where ordinal_number = " . $ordinal_number);
            mysqli_query($conn, $simbad_V_query);
        }
        if(!is_null($simbad_SpType) && !str_contains($simbad_SpType, "~")){
            $simbad_SpType_query = ("update BrightStarMultiple set simbad_SpType = '" . $simbad_SpType . "' where ordinal_number = " . $ordinal_number);
        ...
        mysqli_query($conn, "update BrightStarMultiple set BrightStarMultiple.designation = BrightStarMultiple.simbad_DR3 where BrightStarMultiple.simbad_DR3 in (select Gaia.designation from Gaia)");
    }
