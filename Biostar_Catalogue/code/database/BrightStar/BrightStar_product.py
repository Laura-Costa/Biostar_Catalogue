...
cursor.execute("select {father_key_column}, "
               "V + 5 + 5 * log(10, simbad_parallax / 1000.0) as MV "
               "from {father_table} "
               "where simbad_parallax is not null and "
               "simbad_parallax > 0 and "
               "V is not null".format(father_key_column=father_key_column, father_table=father_table))
value = cursor.fetchall()
...
cursor.execute("select {father_key_column}, "
               "( "
               "abs((V + 5 + 5 * log(10, simbad_parallax/1000.0)) - (V + 5 + 5 * log(10, (simbad_parallax + simbad_parallax_error)/1000.0))) "
               "+ "
               "abs((V + 5 + 5 * log(10, simbad_parallax/1000.0)) - (V + 5 + 5 * log(10, (simbad_parallax - simbad_parallax_error)/1000.0))) "
               ") / 2.0 as MV_error "
               "from {father_table} "
               ...
               "simbad_parallax - simbad_parallax_error > 0".format(father_key_column=father_key_column, father_table=father_table))
value = cursor.fetchall()
for my_tuple in value:
...
cursor.execute("select {father_key_column}, "
               "B_V "
               "from {father_table} "
               "where B_V is not null".format(father_key_column=father_key_column, father_table=father_table))
value = cursor.fetchall()
for my_tuple in value:
