...
cursor.execute("create table {table_name}( "
               ...
               "foreign key (HIP) references Hipparcos29mas(HIP) on delete restrict "
               ")".format(table_name=table_name))

# carregar os dados de table_name.csv na tabela table_name
with open("input_files/Gaia50pc.csv", "r") as csv_file:
    ...
    for line in csv_file:
        ...
        if len(line[3].strip()) == 0 or float(line[3].strip()) < (1.0/50)*1000:
        ...
        # load HIP
        cursor.execute("update {table_name} set HIP = simbad_HIP "
                       "where simbad_HIP in (select HIP from Hipparcos29mas)".format(table_name=table_name))
