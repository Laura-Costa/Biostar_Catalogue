<?php
    ...
    // esse 17 é a chave primária do último registro inserido no SupplementMultiple a partir do
    // registro original do Supplement com /X no HD_Suffix
    $last_ordinal_number = 17;

    // todos os registros com letra(s) no HD_Suffix têm HD não nulo
    $query = mysqli_query($conn, "select ordinal_number, HD, simbad_main_identifier from Supplement where HD_Suffix is not null and HD_Suffix not like '%/%'");
    while($row = mysqli_fetch_array($query)) {
        ...
        // coloca o ordinal_number_Supplement e o main_identifier na tabela SupplementMultiple
        if (is_null($number_of_objects)) {
            ...
            // copiar o main_identifier do Supplement para o SupplementMultiple
            mysqli_query($conn, "insert into SupplementMultiple(simbad_main_identifier) values('" . $simbad_main_identifier . "')");
            $last_ordinal_number++;

            // copiar a chave estrangeira ordinal_number do Supplement para o SupplementMultiple
            mysqli_query($conn, "update SupplementMultiple set ordinal_number_Supplement = '" . $ordinal_number_Supplement . "' where ordinal_number = " . $last_ordinal_number);
