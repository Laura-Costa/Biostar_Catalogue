...
cursor.execute("select {father_table}.designation, "
               ...
               "( "
               "{father_table}.parallax < 50.000 and "
               "{father_table}.parallax + 3*{father_table}.parallax_error >= 50.000 "
               ") and "
               "{son_table}.MG <= 7.57 and "
               "{father_table}.phot_g_mean_mag <= 9.08 "
               "order by phot_g_mean_mag asc".format(father_table=father_table,
                                                                                       son_table=son_table))
...
for i in range(len(parallax_list)):
    if parallax_list[i] is not None:
        ...
        parallax_list[i] = f"{parallax_list[i]:8.5f}"
...
for i in range(len(phot_g_mean_mag_list)):
    if phot_g_mean_mag_list[i] is not None:
        ...
        phot_g_mean_mag_list[i] = f"{phot_g_mean_mag_list[i]:7.5f}"
...
for i in range(len(phot_rp_mean_mag_list)):
    if phot_rp_mean_mag_list[i] is not None:
        ...
        phot_rp_mean_mag_list[i] = f"{phot_rp_mean_mag_list[i]:7.5f}"
...
with open("/home/lh/Desktop/Biostar_Catalogue/Biostar_Catalogue/output_files/CAT1/text_files/tres_estrelas_da_borda.txt", "w") as text_file:
    header = ["designation", "parallax ± parallax_error",
              "phot_g_mean_mag", "phot_bp_mean_mag", "phot_rp_mean_mag",
              "bp_rp"]
    text_file.write("{0[0]:<29}" # designation
                    "{0[1]:<26}" # parallax ± parallax_error
                    "{0[2]:<17}" # phot_g_mean_mag
                    "{0[3]:<17}" # phot_bp_mean_mag
                    "{0[4]:<17}" # phot_rp_mean_mag
                    "{0[5]:<0}" # bp_rp
                    "\n".format(header))
    for (designation, parallax, parallax_error, phot_g_mean_mag, phot_bp_mean_mag, phot_rp_mean_mag, bp_rp) in zip(designation_list, parallax_list, parallax_error_list, phot_g_mean_mag_list, phot_bp_mean_mag_list, phot_rp_mean_mag_list, bp_rp_list):
        ...
        lista = [designation, parallax, mais_ou_menos, parallax_error, phot_g_mean_mag, phot_bp_mean_mag, phot_rp_mean_mag, bp_rp]

        text_file.write("{0[0]:<29}" # designation
                        "{0[1]:<9}" # parallax
                        "{0[2]:<2}" # sinal
                        "{0[3]:<15}" # parallax_error
                        "{0[4]:<17}" # phot_g_mean_mag
                        "{0[5]:<17}" # phot_bp_mean_mag
                        "{0[6]:<17}" # phot_rp_mean_mag
                        "{0[7]:<0}" # bp_rp
                        "\n".format(lista))