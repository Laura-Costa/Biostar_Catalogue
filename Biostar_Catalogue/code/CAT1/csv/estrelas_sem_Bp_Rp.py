...
cursor.execute("""select {father_table}.designation, """
               ...
               """where (phot_bp_mean_mag is null or phot_rp_mean_mag is null) and """
               """{father_table}.designation = {son_table}.designation """
               """into outfile '/var/lib/mysql-files/estrelas_sem_Bp_Rp.csv' """ 
               """fields optionally enclosed by '"' """
               """terminated by ',' """
               """lines terminated by '\n' """.format(father_table=father_table,
                                               son_table=son_table))

file = pd.read_csv('/var/lib/mysql-files/estrelas_sem_Bp_Rp.csv', header=None)
file.to_csv("/home/lh/Desktop/Biostar_Catalogue/Biostar_Catalogue/output_files/CAT1/csv/estrelas_sem_Bp_Rp.csv", header=header, index=None)
os.remove("/var/lib/mysql-files/estrelas_sem_Bp_Rp.csv")