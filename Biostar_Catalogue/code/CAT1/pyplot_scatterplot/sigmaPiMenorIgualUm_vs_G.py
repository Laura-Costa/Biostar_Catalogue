...
def sql_query(y_axis, x_axis):
    ...
    query = ("select {father_table}.simbad_HD, "
             ...
             "{father_table}.{y_axis} <= 1.14".format(father_table=father_table, x_axis=x_axis,
                                                       y_axis=y_axis))
...
f.diagram(cursor, query, query_emphasis, colors, HDs, 'CAT1/pyplot_scatterplot/#/parallax_error_menor_ou_igual_a_um_G.#',
          1.0, 0.1,
          r'$G \; (mag)$', r'$\sigma_{\pi} \; (mas)$', 8.0,
          0.20, 0.20, 0.01, 0.05,
          'CAT1', xrot=0, minortickwidth=0.5, majortickwidth=1.0, dpy=1,
          axeslabelsize=10,
