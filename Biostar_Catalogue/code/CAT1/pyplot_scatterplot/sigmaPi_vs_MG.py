...
def diagram(cursor, query, query_emphasis, colors, hds, path, xgap, ygap, xlabel, ylabel, size, xmargin_left, xmargin_right, ymargin_upper, ymargin_bottom, suptitle, xrot=0, redx=-1, redy=-1):
    ...
    plt.xlim(1.1053998908411284 + 1.1, 21.838439008592815)
...
def sql_query_zoom(y_axis, x_axis):
    ...
    query = ("select {father_table}.simbad_HD, "
             ...
             "{son_table}.{x_axis} is not null and "
             "{father_table}.{y_axis} is not null and "
             "{father_table}.parallax_error > 1.00".format(father_table=father_table, son_table=son_table, x_axis=x_axis,
                                                       y_axis=y_axis))
...
"""
fazer o diagrama de parallax_error x MG
"""
(query, query_emphasis) = sql_query('parallax_error', 'MG')
#f.diagram(cursor, query, query_emphasis, colors, HDs,'CAT1/pyplot_scatterplot/jpg/parallax_error_MG.jpg',
#          1.25, 0.5, r'$$', r'$$', 2.0, 0.20, 0.20,
#          0.20, 0.20, 'CAT1')
f.diagram(cursor, query, query_emphasis, colors, HDs, 'CAT1/pyplot_scatterplot/#/parallax_error_MG.#',
          3.0, 1.0,
          r'$M(G)$', r'$σ(π)$', 8.0,
          0.20, 0.20, 0.20, 0.20,
...
(query, query_emphasis) = sql_query_zoom('parallax_error', 'MG')
diagram(cursor, query, query_emphasis, colors, [],'CAT1/pyplot_scatterplot/jpg/parallax_error_MG_ampliado.jpg',
        3.0, 1.0, r'$M(G)$', 'σ(π)', 6.0, 0.20, 0.20,
        0.20, 0.20, 'CAT1: estrelas com σ(π) > 1.0 (mas)')
