...
create table Produto_Gaia (
     ...
     primary key (ra, declination),
     foreign key (ra, declination) references Source_Gaia(ra, declination)
);
...
create table Produto_Hipparcos(
      ...
      primary key (RAdeg, DEdeg),
      foreign key (RAdeg, DEdeg) references Source_Hipparcos(RAdeg, DEdeg)
);
