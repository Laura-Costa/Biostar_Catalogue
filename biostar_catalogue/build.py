    ...
    for line in csv.reader(tsv, dialect="excel-tab"):
        if len(line) != 0:
            ...
            if len(line[11].strip()) != 0 and float(line[11].strip()) >= 0.039 * 1000.0:
                ...
                if len(BD_value) == 0:
                    cursor.execute("update Hipparcos set BD = NULL where HIP = {}".format(int(HIP_value)))
                else:
                    cursor.execute("update Hipparcos set BD = '{}' where HIP = {}".format(BD_value, int(HIP_value)))

                CoD_value = line[73].strip()
                if len(CoD_value) == 0:
                    cursor.execute("update Hipparcos set CoD = NULL where HIP = {}".format(int(HIP_value)))
                else:
                    cursor.execute("update Hipparcos set CoD = '{}' where HIP = {}".format(CoD_value, int(HIP_value)))

                CPD_value = line[74].strip()
                if len(CPD_value) == 0:
                    cursor.execute("update Hipparcos set CPD = NULL where HIP = {}".format(int(HIP_value)))
                else:
                    cursor.execute("update Hipparcos set CPD = '{}' where HIP = {}".format(CPD_value, int(HIP_value)))
                ...
                if len(SpType_value) == 0:
                    cursor.execute("update Hipparcos set SpType = NULL where HIP = {}".format(int(HIP_value)))
                else:
                    cursor.execute("update Hipparcos set SpType = '{}' where HIP = {}".format(SpType_value, int(HIP_value)))
tsv.close()
