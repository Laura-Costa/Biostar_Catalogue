...
# Criar o diagrama BrightStar_plus_Supplement_MV_versus_B_minus_V.pdf
...
cursor.execute("(select TRIM(BrightStar.simbad_parallax) + 0, "
               ...
               "UNION ALL "
               "select TRIM(BrightStarSupplement.simbad_parallax) + 0, "
               "TRIM(BrightStarSupplement.B_V) + 0, "
               "TRIM(BrightStarSupplement.V + 5 + 5*log(10, BrightStarSupplement.simbad_parallax/1000.0))+0 as MV "
               "from BrightStarSupplement "
               "where BrightStarSupplement.simbad_parallax is not null and "
               "BrightStarSupplement.B_V is not null and "
               "BrightStarSupplement.V is not null ")
...
fig.suptitle("Bright Star + Supplement: ${}$ estrelas em um raio de ${:.4f}$pc $(π ≥ {:.7f}'')$".format(len(value), 1.0 / (min_simbad_parallax / 1000.0), min_simbad_parallax / 1000.0), fontsize=8)
...
# salvar diagrama
plt.savefig('/home/lh/Desktop/Catalogo_GAIA/biostar_catalogue/files/BrightStar_plus_Supplement/diagram/BrightStar_plus_Supplement_MV_versus_B_minus_V.pdf')

# close matplotlib.pyplot as plt object
plt.close()

# Criar o diagrama BrightStar_plus_Supplement_without_designation_DR3_in_Simbad_MV_versus_B_minus_V.pdf
...
cursor.execute("(select TRIM(BrightStar.simbad_parallax) + 0, "
               ...
               "where BrightStar.simbad_parallax is not null and "
               "BrightStar.B_V is not null and "
               "BrightStar.V is not null and "
               "BrightStar.simbad_designation_DR3 is null) "
               "UNION ALL "
               "(select TRIM(BrightStarSupplement.simbad_parallax) + 0, "
               "TRIM(BrightStarSupplement.B_V) + 0, "
               "TRIM(BrightStarSupplement.V + 5 + 5*log(10, BrightStarSupplement.simbad_parallax/1000.0))+0 as MV "
               ...
               "where BrightStarSupplement.simbad_parallax is not null and "
               "BrightStarSupplement.B_V is not null and "
               "BrightStarSupplement.V is not null and "
               "BrightStarSupplement.simbad_designation_DR3 is null) ")
value = cursor.fetchall()
...
plt.xlim(min(x_axis) - 0.15, max(x_axis) + 0.15)
plt.ylim(max(y_axis) + 0.2, min(y_axis) - 0.2)
fig.suptitle("Bright Star + Supplement: estrelas sem DR3 no Simbad", fontsize=8)
ax.set_title("${}$ estrelas em um raio de ${:.4f}$pc, $π ≥ {:.7f}''$ (Simbad parallax)".format(len(value), 1.0 / (min_simbad_parallax / 1000.0), min_simbad_parallax / 1000.0), fontsize=5)
plt.xlabel("$B-V$ (mag)", fontsize=7)
plt.ylabel("$M(V)$ (mag)", fontsize=7)

# definir os intervalos dos minor e major ticks, dos eixos x e eixos y
ax.xaxis.set_major_locator(MultipleLocator(0.125))
ax.xaxis.set_minor_locator(MultipleLocator(0.125/9))
ax.yaxis.set_major_locator(MultipleLocator(1))
ax.yaxis.set_minor_locator(MultipleLocator(1/9))
...
# salvar diagrama
plt.savefig('/home/lh/Desktop/Catalogo_GAIA/biostar_catalogue/files/BrightStar_plus_Supplement/diagram/BrightStar_plus_Supplement_without_designation_DR3_in_Simbad_MV_versus_B_minus_V.pdf')
