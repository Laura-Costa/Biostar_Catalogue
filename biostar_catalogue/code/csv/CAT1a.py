...
cursor.execute('''select CAT1.designation, '''
               ...
               '''into outfile '/var/lib/mysql-files/CAT1a.csv' '''
               '''fields optionally enclosed by '"' terminated by ',' LINES TERMINATED BY '\n' ''')

file = pd.read_csv("/var/lib/mysql-files/CAT1a.csv", header=None)
file.to_csv("/home/lh/Desktop/Catalogo_GAIA/biostar_catalogue/files/CAT1a/csv/CAT1a.csv", header=header_list, index=False)
os.remove("/var/lib/mysql-files/CAT1a.csv")

# Criando o arquivo CAT1a_interesec_Hipparcos_completo.csv
...
cursor.execute('''select CAT1.designation, '''
               ...
               '''from CAT1, CAT1_product, Hipparcos_completo '''
               '''where CAT1.designation = CAT1_product.designation and ''' 
               '''((CAT1.parallax >= 50.00) or ''' 
               '''(CAT1.parallax < 50.00 and (CAT1.parallax + CAT1.parallax_error >= 50.00))) and '''
               '''CAT1.HIP = Hipparcos_completo.HIP '''
               '''order by CAT1.parallax asc '''
               '''into outfile '/var/lib/mysql-files/CAT1a_interesec_Hipparcos_completo.csv' '''
               '''fields optionally enclosed by '"' terminated by ',' LINES TERMINATED BY '\n' ''')
...
file = pd.read_csv("/var/lib/mysql-files/CAT1a_interesec_Hipparcos_completo.csv", header=None)
file.to_csv("/home/lh/Desktop/Catalogo_GAIA/biostar_catalogue/files/CAT1a/csv/CAT1a_interesec_Hipparcos_completo.csv", header=header_list, index=False)
os.remove("/var/lib/mysql-files/CAT1a_interesec_Hipparcos_completo.csv")
